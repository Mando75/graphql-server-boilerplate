"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
exports.__esModule = true;
var apollo_server_1 = require("apollo-server");
var makeSchema_1 = require("./makeSchema");
var CreateTypeORMConnection_1 = require("./CreateTypeORMConnection");
var mergeSchemas_1 = require("graphql-tools/dist/stitching/mergeSchemas");
// @ts-ignore
/**
 * Try to bootstrap a database and server connection. If successful,
 * returns the db and server connection instances in an object
 * @param port -> Http server will liston on this port
 */
exports.bootstrapConnections = function (port) { return __awaiter(_this, void 0, void 0, function () {
    var db, app, schemas, server, e_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 4, , 5]);
                return [4 /*yield*/, CreateTypeORMConnection_1.CreateTypeORMConnection()];
            case 1:
                db = _a.sent();
                return [4 /*yield*/, makeSchema_1.makeSchema()];
            case 2:
                schemas = _a.sent();
                server = new apollo_server_1.ApolloServer({
                    schema: mergeSchemas_1["default"]({ schemas: schemas }),
                    formatError: function (error) {
                        console.log(error);
                        return error;
                    },
                    formatResponse: function (response) {
                        console.log(response);
                        return response;
                    }
                });
                console.log("Connected to db " + db.name);
                return [4 /*yield*/, server.listen({
                        port: port
                    })];
            case 3:
                app = _a.sent();
                console.log("\uD83D\uDE80  Server ready at " + app.url + ": Happy Coding!");
                return [2 /*return*/, { app: app, db: db }];
            case 4:
                e_1 = _a.sent();
                console.error("Could not bootstrap server connections. Exiting", e_1);
                process.exit(1);
                return [2 /*return*/, null];
            case 5: return [2 /*return*/];
        }
    });
}); };
/**
 * Attempt to normalize port value
 * @param val
 */
exports.normalizePort = function (val) {
    var port = parseInt(val, 10);
    if (isNaN(port))
        return val;
    if (port >= 0)
        return port;
    return false;
};
